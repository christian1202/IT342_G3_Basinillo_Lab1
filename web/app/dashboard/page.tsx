...
/**
 * DashboardPage — displays real-time status data fetched from the
 * Spring Boot backend at GET /api/dashboard/status.
 *
 * Component states:
 *   1. Loading  — spinner while the fetch is in flight
 *   2. Error    — connection/server failure with a retry button
 *   3. Success  — status card showing backend response data
 */
export default function DashboardPage() {
  ...
  /* ---- fetch helper (reusable for initial load + retry) ---- */
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-indigo-50 to-violet-50 dark:from-slate-950 dark:via-indigo-950/30 dark:to-violet-950/20 px-4 py-12">
      {/* ---- decorative blobs (consistent with login page) ---- */}
